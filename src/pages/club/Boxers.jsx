...
export default function ClubBoxers() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (editing) {
        await api(`/boxers?id=${editing._id}`, { method: 'PUT', body: payload })
        toast('Boxer updated')
      } else {
        await api('/boxers', { method: 'POST', body: payload })
        toast('Boxer added')
      }
      setModalOpen(false)
      load()
      ...
      <Card>
        <CardBody className="p-0">
          {!boxers ? (
            ...
            <Empty title="No boxers yet" message="Add boxers to your club's database to register them for events." action={<Button onClick={openCreate}>Add your first boxer</Button>} />
          ) : (
